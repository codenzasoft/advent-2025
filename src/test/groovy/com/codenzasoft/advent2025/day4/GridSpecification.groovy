...
class GridSpecification extends Specification {
    ...
    def "test finding entries with fewer than 4 adjacent filled (paper roll) entries"() {
        ...
        var grid = new Grid.StringGridBuilder().withRows(rows).build()
    ...
    def "test finding entries"() {
        ...
        then:
        grid.getEntry(new Position(0,0)).get().value() == GridValue.EMPTY
        ...
        grid.getEntry(new Position(9,8)).get().value() == GridValue.PAPER_ROLL
    }
    ...
    def "test finding matching adjacent entries"() {
        ...
        then:
        grid.getMatchingAdjacentPositions(new Position(0,0), GridValue.PAPER_ROLL ).size() == 2
        grid.getMatchingAdjacentPositions(new Position(0,0), GridValue.EMPTY ).size() == 1
        grid.getMatchingAdjacentPositions(new Position(4,4), GridValue.PAPER_ROLL ).size() == 8
    }
